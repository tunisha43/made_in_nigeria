...
export default function MarketplaceResults({ businesses }: MarketplaceResultsProps) {
  ...
  // Category/state apply first, independent of which tab is active --
  // e.g. "Verified" + "Lagos" together means verified businesses in Lagos.
  const scoped = businesses.filter(
    (b) => (category === 'All' || b.category === category) && (state === 'All Nigeria' || b.state === state)
  );
